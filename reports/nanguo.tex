...
图~\ref{fig:pop-flow}--\ref{fig:soundtrack-flow} 对比了三种典型画布。流行人声需要作词，并在旋律、和声、节奏之后编曲；古典器乐和影视配乐跳过作词及常规鼓点路径，围绕主题旋律、和声张力、配器和音色展开。电子器乐则把 Rhythm 放到 Melody 之前，先建立 Groove 与能量骨架。画布显示面向用户的核心创作节点，后端实际执行还包括前置参考分析以及古典路径的可选乐谱导出。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/古典器乐编排.png}
    \caption{古典器乐工作流：跳过作词并保留乐谱导出能力}
    \label{fig:classical-flow}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{L{1.2cm}L{4.0cm}Y}
        ...
        E3 & 古典器乐与影视配乐 & 均跳过 Lyrics；古典满足 ScoreSpec 时可导出乐谱；影视侧重叙事和空间音色。 \\
...
E1 中，人声任务的画布由指挥进入作词，再逐步进入作曲、和声、节奏与编曲，说明歌词不是在最后附加的一段文本，而是旋律与编曲的上游状态。E2 中，雨夜电子纯音乐自动选择电子器乐乐团，节奏节点提前建立 Groove，完整工作台显示状态经历建档、参考、生成和完成；最终获得一个 12 秒 Demo 与两条 3:59 音频。E3 中，古典与影视路径均正确跳过 Lyrics，避免器乐提示词混入演唱要求。
