...
- React 工作区提供对话入口、本地作品集、后台生成进度、项目恢复、精确创作表单和工作流画布。
...
前端默认进入 Chat Agent 对话。Chat Agent 只会选择白名单动作与预设工作流；当用户要求开始创作时，后端立即返回 run id，并在本地后台线程中执行工作流。前端轮询 Run 状态显示阶段进度，生成完成后的 mp3 会保存到 `works/` 根目录并出现在作品集中。创作台仍可直接填写精确参数。
